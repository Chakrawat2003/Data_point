...
const S11_ICON=L.divIcon({className:"",html:`<img class="s11-icon" src="7-11-logo.png" alt="7-Eleven">`,iconSize:[22,22],iconAnchor:[11,11],popupAnchor:[0,-14]});
const S11={all:[],on:localStorage.getItem("pinmap-s11-on")!=="0"};
function s11ZoomSize(z){return Math.round(Math.min(32,Math.max(12,(z-4)*2.3)))}
function s11ApplyZoomSize(){document.documentElement.style.setProperty("--s11-size",s11ZoomSize(map.getZoom())+"px")}
map.on("zoomend",s11ApplyZoomSize);s11ApplyZoomSize();

function s11Build(){
  if(S11.all.length||!window.SEVEN11_DATA)return;
  S11.all=window.SEVEN11_DATA.map(rec=>{
    const m=L.marker([rec.y,rec.x],{icon:S11_ICON});
    m.bindPopup(`<div class="pin-title">🏪 ${esc(rec.n)}</div>
      <div>${esc(rec.a)}</div>
      <div class="pin-meta"><a href="https://www.google.com/maps/dir/?api=1&destination=${rec.y},${rec.x}" target="_blank" rel="noopener">🧭 นำทาง</a> · ${rec.y.toFixed(5)}, ${rec.x.toFixed(5)}</div>`);
    return{rec,m};
  });
}
function s11Filter(q){
  if(!q)return S11.all;
  return S11.all.filter(x=>(x.rec.n+" "+x.rec.a).toLowerCase().includes(q));
}
function s11Update(){
  s11Build();
  const q=$("search").value.toLowerCase().trim();
  const list=q?s11Filter(q):(S11.on?S11.all:[]);
  ...
  $("s11Count").textContent=list.length?list.length.toLocaleString("th-TH"):"";
  $("s11Toggle").classList.toggle("active",list.length>0);
}
$("s11Toggle").onclick=()=>{
  S11.on=!S11.on;localStorage.setItem("pinmap-s11-on",S11.on?"1":"0");s11Update();
  if(S11.on)toast("แสดงร้าน 7-Eleven "+S11.all.length.toLocaleString("th-TH")+" จุด");else toast("ซ่อนร้าน 7-Eleven แล้ว");
};
s11Update();
...
$("search").oninput=()=>{rebuild();s11Update()};
